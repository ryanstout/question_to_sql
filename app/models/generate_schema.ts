import { Query, TableNames, Columns, Cardinality, ColumnValues, Count } from "~/models/query.server"
    ...
    for await (const tableName of tableNames) {
        ...
        for await (const column of columns) {
            ...
            const skip = (
                ...
                (tableName === 'orders' && columnName === 'status') || // weird field we don't use anymore
            ...
            if (!skip) {
                ...
                const forceIncludes = ['shipping_state', 'shipping_country', 'shipping_name'].includes(columnName)

                if ((dataType === 'character varying' && columnName.indexOf('"') === -1) || forceIncludes) {
                    try {
                        ...
                        if (cardinalityInt < 150 || forceIncludes) {
                            const columnValues = await ColumnValues(tableName, columnName, limit)
                            const columnValuesCSV = columnValues.map((columnValue) => {
                                return JSON.stringify(columnValue[columnName])
                            })

                            if (cardinalityInt > limit) {
                                cardComment = ` -- values include but are not limited to: ${columnValuesCSV.slice(0, limit).join(", ")}...`
                            } else {
                ...
                console.log(`${comment}${commentSpaces}${columnName} ${dataType}${defaultStr}${nullableStr}${comma}${cardComment}`)
            }
