...
export class UserDb {
    ...
    async query(query: string): Promise<QueryResult> {
        ...
        const fieldTypes = new Map(queryResult.fields!.map((field) => { return [field.fieldName, field.dataTypeName] }))
    ...
    async TableNames(): Promise<string[]> {
        const tables = await this.sql`SELECT table_name FROM information_schema.tables WHERE table_schema='public'`
    ...
    async Columns(tableName: string): Promise<postgres.RowList<postgres.Row[]>> {
        const columns = await this.sql`SELECT * FROM information_schema.columns WHERE table_name = ${tableName};`
        return columns
    }
    async ColumnValues(tableName: string, columnName: string, limit: number = 1000): Promise<Map<string, string>> {
    ...
    async Cardinality(tableName: string, columnName: string): Promise<Map<string, string>> {
