...
import { IconChartBar, IconChartLine, IconGrain, IconNotes } from "@tabler/icons";
...
function BarChart({ type, data }: { type: string, data: BarDatum[] }) {
    ...
    return (
        <div style={{ height: '500px' }}>
            ...
            <ResponsiveBar
                data={data}
                keys={keys.slice(1)} indexBy={keys[0]}
                enableGridX={false}
                enableLabel={false}
                padding={0.2}
                margin={{ top: 50, right: 130, bottom: 50, left: 80 }}
...
export default function VisualizeMethod() {
    ...
    const match = useMatches().find(match => match.id === 'routes/query/$questionGroupId')
    ...
    function changeTab(value: string | null) {
        if (value) {
            navigate(`/query/${params.questionGroupId}/chart/${value}?q=` + encodeURIComponent(question), { preventScrollReset: true })
        }
    }
    ...
    return (
        <Box>
            <Tabs defaultValue={params.chart} onTabChange={v => changeTab(v)}>
                <Tabs.List>
                    <Tabs.Tab value="bar" icon={<IconChartBar size={14} />}>Bar</Tabs.Tab>
                    <Tabs.Tab value="line" icon={<IconChartLine size={14} />}>Line</Tabs.Tab>
                    <Tabs.Tab value="scatter" icon={<IconGrain size={14} />}>Scatter</Tabs.Tab>
                    <Tabs.Tab value="raw" icon={<IconNotes size={14} />}>Raw</Tabs.Tab>
                </Tabs.List>
