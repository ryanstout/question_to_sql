...
import { GenerateSqlQuery, EnhanceSqlQuery } from '~/models/language_model/query_generator.server'
...
async function findOrCreateQuestion(userId: number, q: string, questionGroupId: number) {
...
export async function action({ request }: ActionArgs) {
    ...
    if (action_name === 'update') {
    ...
    } else if (action_name === 'mark_correct' || action_name === 'mark_incorrect') {
        ...
        await prisma.question.update({ data: { correctState: correctValue }, where: { id: question.id } })
    } else {
        // actionName === 'create'
    ...
    if (q) {
        return redirect(`/query/${questionGroup.id}/chart/raw?q=${encodeURIComponent(q || '')}`)
    } else {
...
export let loader: LoaderFunction = async ({ params, request }): Promise<LoaderData> => {
    ...
    const { questionGroupId } = zx.parseParams(params, { questionGroupId: zx.NumAsString })
                ...
                <Grid sx={{ height: '100%' }} p="0" m="0">
                            ...
                            <Box>
                                <Form method="post">
                                    <Flex>
                                        ...
                                        <TextInput size="lg" w="100%" name="q" value={question} onChange={e => setQuestion(e.target.value)} />
                                        <Button size="lg" type="submit">&gt;</Button>
                                        {navigation.state === 'submitting' && <Loader size="xl" variant="bars" p="10px" />}
                                    </Flex>
                                </Form>
                            ...
                            <Flex>
                                <Box w="100%">
                                    <Form method="post" action="/query">
                                        <Button type="submit" w="100%">New Question</Button>
                                    </Form>
                                </Box>
                                <Box w="100%">
                                    <Form method="post">
                    ...
                    <Grid.Col span={6} p="0" sx={{ position: 'relative' }}>
                        ...
                        <Box sx={{ position: 'absolute', bottom: '0px', left: '0px', right: '0px' }}>
                            <Text c="dimmed" align="center"><span style={{ padding: '5px', backgroundColor: 'rgba(128,128,128,0.1)' }}>Press Shift+Enter to executre an updated query</span></Text>
                        </Box>
                    </Grid.Col>
                </Grid>
            </Header >
