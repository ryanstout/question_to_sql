...
class Import():
    def __init__(self, user_id, database):
        ...
        logger.debug("starting import", database=database, user_id=user_id)
    ...
    def create_column_records(self, table_description: DataSourceTableDescription):
        ...
        for column in columnInfo:
            ...
            if not any(re.search(regex, name) for regex in SKIP_COLUMNS):
                kind = column['kind']
                type = column['type']

                logger.info(f"inspecting column", name=name, type=type)
