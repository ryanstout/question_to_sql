...
class ValuesRankerModel(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = nn.Sequential(nn.Linear(2, 50), nn.ReLU(), nn.Linear(50, 1))
    ...
    @classmethod
    def train_model(cls):
        ranker_data = RankerDataModule("values", batch_size=256)

        # model
        model = ValuesRankerModel()

        # training
        trainer = pl.Trainer(
            default_root_dir=f"{ranker_models_path()}/values_ranker"
