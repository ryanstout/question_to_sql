...
function copyToClipboard(text: string) {
  require('child_process').spawnSync('/usr/bin/pbcopy', [], {
    ...
    input: JSON.stringify(tableMap, undefined, 2)
  })
}
...
function isDebugEnabled() {
  ...
  const inspectPassedOverCLI = process.execArgv.filter(a => a.includes('--inspect-brk') || a == 'inspect').length > 0
...
async function describeTableAsSQL(snowflake: Snowflake, fullyQualifiedTableName: string): Promise<string> {
  const result = await executeSQL<{[key: string]: string}>(snowflake, `SELECT GET_DDL('TABLE', '${fullyQualifiedTableName}');`)
...
async function describeTable(snowflake: Snowflake, fullyQualifiedTableName: string) {
  const result = await executeSQL(snowflake, `DESCRIBE TABLE ${fullyQualifiedTableName};`)
  return result
}
...
async function describeTables(snowflake: Snowflake, database: string) {
  ...
  const snowflakeTableList = await executeSQL<TableDescription>(snowflake, showTablesSql)
